...
def run_single_sample(
    ...
    for combination_num, combination in enumerate(RULES_COMBINATIONS[:num_combinations_to_test]):
        ...
        # Run counterfactual generation ONCE, getting num_best_results counterfactuals
        num_best_results = getattr(config.experiment_params, 'num_best_results', 5)
        print(f"INFO: Running counterfactual generation once with num_best_results={num_best_results}")
        
        # Prepare argument for single run
        run_args = (
            0,  # replication_num is always 0 now
            ORIGINAL_SAMPLE,
            TARGET_CLASS,
            FEATURES_NAMES,
            dict_non_actionable,
        ...
        total_attempts += 1
        result = _single_run(run_args)
        
        # Process result from single run (which returns multiple counterfactuals via num_best_results)
        if result is None:
        ...
        # Get all counterfactuals from single run (num_best_results)
        ...
        if not all_counterfactuals:
            ...
            combination_viz['results'].append(result_viz)
            
            # Add all counterfactuals to dataframe with rules
            for cf_idx, counterfactual in enumerate(all_counterfactuals):
                ...
                counterfactuals_df_results.append(cf_data)
            ...
            if wandb_run:
                ...
                # Create series for this run
                if cf_model.best_fitness_list and cf_model.average_fitness_list:
                    run_key = f"s{SAMPLE_ID}_c{combination_num}"
                    for gen, (best, avg) in enumerate(zip(cf_model.best_fitness_list, cf_model.average_fitness_list)):
                        wandb.log({
                            "generation": gen,
                            f"fitness/best_{run_key}": best,
                            f"fitness/avg_{run_key}": avg,
                        })
                        
                    # Also log metadata about this fitness series to summary
                    wandb.run.summary[f"fitness_metadata/{run_key}/sample_id"] = SAMPLE_ID
                    wandb.run.summary[f"fitness_metadata/{run_key}/combination"] = get_combination_label(combination, dict_non_actionable)
                    wandb.run.summary[f"fitness_metadata/{run_key}/final_fitness"] = best_fitness
                    wandb.run.summary[f"fitness_metadata/{run_key}/generations"] = len(cf_model.best_fitness_list)
        ...
        if counterfactuals_df_results:
            counterfactuals_df_combinations.extend(counterfactuals_df_results)
        
        # Compute combination-level comprehensive metrics
        combination_comprehensive_metrics = {}
        if combination_viz['results'] and COMPREHENSIVE_METRICS_AVAILABLE:
            try:
                x_original = np.array([ORIGINAL_SAMPLE[feat] for feat in FEATURES_NAMES])
                result_viz = combination_viz['results'][0]
                cf_list = [np.array([cf[feat] for feat in FEATURES_NAMES]) 
                          for cf in result_viz['all_counterfactuals']]
        ...
        if combination_viz['results'] and CF_EVAL_AVAILABLE and wandb_run:
            try:
                x_original = np.array([ORIGINAL_SAMPLE[feat] for feat in FEATURES_NAMES])
                # Get all counterfactuals from the single run
                result_viz = combination_viz['results'][0]
                cf_list = [np.array([cf[feat] for feat in FEATURES_NAMES]) 
                          for cf in result_viz['all_counterfactuals']]
                cf_array = np.array(cf_list)
    ...
    success_rate = valid_counterfactuals / total_attempts if total_attempts > 0 else 0.0
    ...
    if wandb_run:
        ...
        wandb.run.summary[f"sample_{SAMPLE_ID}/total_attempts"] = total_attempts
    ...
    for combination_viz in visualizations:
        combo_copy = {'label': combination_viz['label'], 'results': []}
        for result_viz in combination_viz['results']:
            best_fitness_list = getattr(result_viz['cf_model'], 'best_fitness_list', [])
            average_fitness_list = getattr(result_viz['cf_model'], 'average_fitness_list', [])
            ...
            combo_copy['results'].append(result_copy)
    ...
    if getattr(config.output, 'save_visualizations', True) if hasattr(config, 'output') else True:
        for combination_idx, combination_viz in enumerate(visualizations):
            ...
            for cf_idx, counterfactual in enumerate(all_counterfactuals):
                ...
                if heatmap_fig or comparison_fig:
                    ...
                    result_viz['explanations'] = explanations
                    
                    # Save explanations locally as text file
                    if getattr(config.output, 'save_visualization_images', False):
                        os.makedirs(sample_dir, exist_ok=True)
                        explanation_text = f"""Sample {SAMPLE_ID} - Combination {combination_idx} - Counterfactual {cf_idx}
...
"""
                        explanation_path = os.path.join(sample_dir, f'explanation_combo_{combination_idx}_cf_{cf_idx}.txt')
...
"""
                    ...
                    if getattr(config.output, 'save_visualization_images', False):
                            ...
                            if pca_fig:
                                ...
                                try:
                                    ...
                                    # Get evolution history from single run
                                    result_viz = combination_viz['results'][0]
                                    evolution_history = result_viz.get('evolution_history', [])
                                    if evolution_history:
                                        for gen_idx, gen_sample in enumerate(evolution_history):
                                            ...
                                            gen_row = {'replication': 'run', 'generation': gen_idx + 1, 'predicted_class': gen_pred_class}
                                        ...
                                        if n_pcs >= 2:
                                            ...
                                            if evolution_history:
                                                for gen_idx, gen_sample in enumerate(evolution_history):
                                                    ...
                                                    if is_final:
                                                        # For each final counterfactual (multiple from num_best_results)
                                                        for cf_idx, cf in enumerate(result_viz.get('all_counterfactuals', [])):
                                                            cf_numeric = pd.DataFrame([cf])[FEATURE_NAMES_LOCAL].select_dtypes(include=[np.number])
                                                            cf_scaled = scaler_pp.transform(cf_numeric)
                                                            cf_pcs = pca_pp.transform(cf_scaled)
                                                            cf_pc_row = {pc_names[i]: cf_pcs[0, i] for i in range(n_pcs)}
                                                            cf_pc_row['point_type'] = 'Counterfactual'
                                                            cf_pc_rows.append(cf_pc_row)
                                                    else:
                                            ...
                                            for i, feat_y in enumerate(features_to_plot):
                                                for j, feat_x in enumerate(features_to_plot):
                                                    ...
                                                    if i == j:
                                                        ...
                                                        # Plot evolution from single run
                                                        # Get evolution history from single run
                                                        result_viz = combination_viz['results'][0]
                                                        evolution_history = result_viz.get('evolution_history', [])
                                                        if evolution_history:
    ...
    try:
        # Collect all result-level metrics (for each CF)
        cf_metrics_list = []
        for combination_idx, combination_viz in enumerate(visualizations):
            if combination_viz['results']:
                for result_viz in combination_viz['results']:
                    if 'metrics' in result_viz:
                        metrics_row = {
                            'sample_id': SAMPLE_ID,
                            'combination_idx': combination_idx,
                            'combination': str(combination_viz['label']),
                        }
                        metrics_row.update(result_viz['metrics'])
                        cf_metrics_list.append(metrics_row)
        
        if cf_metrics_list:
            cf_metrics_df = pd.DataFrame(cf_metrics_list)
            cf_metrics_csv = os.path.join(sample_dir, 'cf_metrics.csv')
            cf_metrics_df.to_csv(cf_metrics_csv, index=False)
            print(f"INFO: Saved CF metrics to {cf_metrics_csv}")
    ...
    print(f"INFO: Completed sample {SAMPLE_ID}: {valid_counterfactuals}/{total_attempts*len(all_counterfactuals) if total_attempts > 0 and 'result' in locals() else 0} total counterfactuals generated")
...
def run_experiment(config: DictConfig, wandb_run=None):
    ...
    try:
        ...
        for r in results:
            ...
            # Load CF metrics (new name)
            cf_csv = os.path.join(sample_dir, 'cf_metrics.csv')
            if os.path.exists(cf_csv):
                cf_df = pd.read_csv(cf_csv)
                all_cf_metrics.append(cf_df)
        ...
        if all_cf_metrics:
            aggregated_cf_df = pd.concat(all_cf_metrics, ignore_index=True)
            aggregated_cf_csv = os.path.join(experiment_dir, 'all_cf_metrics.csv')
            aggregated_cf_df.to_csv(aggregated_cf_csv, index=False)
            print(f"INFO: Saved aggregated CF metrics to {aggregated_cf_csv}")
